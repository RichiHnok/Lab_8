...
// Получить поток вывода для HTTP-ответа
        ...
        pw.println("<html><head><title>Chad!</title>" +
                "<meta httpequiv='Content-Type'" +
                " content='text/html; charset=utf-8'/></head>");
// Если возникла ошибка - сообщить о ней
        if (errorMessage!=null) {
            pw.println("<p><font color='red'>" +
                    errorMessage + "</font></p>");
        }
// Вывести форму
        pw.println("<form action='/chat/' " +
                "method='post'>Enter name: " +
...
// Пустое имя недопустимо - сообщить об ошибке
            errorMessage = "Name of user cant be empty!";
        } else {
...
// Обновить идентификатор сессии пользователя в cookies
            ...
            String joinMessage = name + " has joined chat.";
            ChatUser system = activeUsers.get("system");
            system.setSessionId(request.getSession().getId());
            synchronized (messages){
                messages.add(new ChatMessage(joinMessage, system, Calendar.getInstance().getTimeInMillis()));
            }
...
// Извиниться, отказать и попросить ввести другое имя
            return "Sorry, <strong>" + name + "</strong> already in use. Choose another one!";
        }
    }
}
